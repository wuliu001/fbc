...
/*Event structure for Event `control_end_queue` */;

DROP EVENT IF EXISTS `control_end_queue`;

DELIMITER $$

CREATE EVENT `control_end_queue` ON SCHEDULE EVERY 1 MINUTE STARTS '2016-01-01 00:00:00' ON COMPLETION PRESERVE ENABLE DO ll:BEGIN
    DECLARE v_returnCode                 INT;
    DECLARE v_returnMsg                  LONGTEXT;
    DECLARE v_curMsg                     LONGTEXT;
    DECLARE imp_lock                     INT;
    DECLARE v_procname                   VARCHAR(50) DEFAULT 'control_end_queue.job';
    DECLARE v_modulename                 VARCHAR(50) DEFAULT 'messageManager';
    DECLARE v_body                       LONGTEXT DEFAULT NULL;
    DECLARE v_lock_name                  VARCHAR(50) DEFAULT 'control_end_queue';
    ...
    DECLARE cur1 CURSOR FOR SELECT queue_type, queue_step
    ...
    OPEN cur1;
    myrepeat:REPEAT
        FETCH cur1 INTO v_queue_type,v_queue_step;
        IF NOT done THEN
            ...
            SET v_sql = CONCAT('
                        UPDATE msg_queues.queues
                           SET is_delete = 1,last_update_time = UTC_TIMESTAMP()
                         WHERE id IN (', v_id, ')');

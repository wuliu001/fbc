...
ll:BEGIN
    ...
    DECLARE cur_queue_type CURSOR FOR SELECT queue_id, queue_type, queue_step, main_queue_info, sub_queue_type, queue_msg
    ...
    SET returnMsg_o = 'select queue_type, queue_step, sub_queue_type infomation.';
    SELECT GROUP_CONCAT('(', 
                        b.queue_id, ',''', 
                        a.queue_type,''',', 
                        a.queue_step, ',''', 
                        MD5(CONCAT(b.id,b.queue_id,b.queue_type,b.queue_step,IFNULL(b.dst_endpoint_info,''),b.create_time)),''',''', 
                        a.sub_queue_type,''',',
                        '"(NULL,''',MD5(CONCAT(b.id,b.queue_id,b.queue_type,b.queue_step,IFNULL(b.dst_endpoint_info,''),b.create_time)),''',''',b.queues,''',',0,')")')
      INTO v_sub_queue_info
      FROM  (
                SELECT DISTINCT queue_type, queue_step, sub_queue_type, is_end_step
    ...
    IF IFNULL(v_sub_queue_info,'')<>'' THEN
        ...
        SET v_sql = CONCAT('INSERT INTO msg_queues.`tmp_sub_queue_type`(`queue_id`,`queue_type`,`queue_step`,`main_queue_info`,`sub_queue_type`,`queue_msg`) VALUES ',v_sub_queue_info);
    ...
    OPEN cur_queue_type;
    lp:LOOP
        FETCH cur_queue_type INTO v_queue_id, v_queue_type, v_queue_step,v_main_queue_info, v_sub_queue_type, v_queue_msg;
        IF done THEN
           LEAVE lp;
        END IF;

...
/*Procedure structure for Procedure `log_module.d` */;

DROP PROCEDURE IF EXISTS `log_module.d`;

DELIMITER $$
CREATE DEFINER=`dba`@`%` PROCEDURE `log_module.d`(user_i            INT,
                         module_i          VARCHAR(50),
...
ll:BEGIN
    ...
    IF JSON_VALID(v_params) <> 1 THEN
        CALL commons.`log_module._insert`(user_i,3,v_module,v_procName,'{}',v_body,v_returnMeg,v_code,v_msg);
    ELSE
        CALL commons.`log_module._insert`(user_i,3,v_module,v_procName,v_params,v_body,v_returnMeg,v_code,v_msg);
    END IF;

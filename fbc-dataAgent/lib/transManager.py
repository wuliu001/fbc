...
def goodsRegister(data_service_host, body):
    ...
    # body check
    check_result, check_msg = misc_utility.bodyChecker(body, body_key_check_dict)
    ...
    if check_result:
        ...
        if body_list_len == 2:
            ...
            flag, goods_info_hash, verify_message = crypto_utility.verify_private_key(user, private_key, data_service_host, '/users/sync/', goods_info)
            if flag is False:
        ...
        # check md5 signature
            ...
            flag, verify_message = crypto_utility.verify_md5_signature(user, goods_info_hash, data_service_host, '/users/sync/', node_dns, goods_info)
            if flag is False:
                api_result = verify_message
                return '200 OK', [('Content-Type', 'text/html')], [api_result + '\n']

        if 'paymentMinStage' not in goods_info_json:
            goods_info_json['paymentMinStage'] = 1
        if 'paymentMaxStage' not in goods_info_json:
            goods_info_json['paymentMaxStage'] = 1
        goods_info = json.dumps(goods_info_json)
        ...
        http_code, api_code, api_result = restful_utility.restful_runner(server_url, 'POST', None, goods_info)
        return '200 OK', [('Content-Type', 'text/html')], [api_result + '\n']
...
def goodsPriceModify(data_service_host, query_string, body):
    ...
    if check_result:
        ...
        # check md5 signature
            ...
            flag, is_create, verify_message = crypto_utility.verify_md5_signature(user, goods_info_hash, data_service_host, '/users/sync/', node_dns, goods_info)
            if flag is False:
                api_result = verify_message
                return '200 OK', [('Content-Type', 'text/html')], [api_result + '\n']
        ...
        http_code, api_code, api_result = restful_utility.restful_runner(server_url, 'PUT', None, goods_info)
        return '200 OK', [('Content-Type', 'text/html')], [api_result + '\n']
